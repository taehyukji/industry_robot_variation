...
function enforcePayloadLimit() {
  ...
  workingInput.max = String(maxPayload);
  const workingPayload = Number(workingInput.value);
  if (Number.isFinite(workingPayload) && workingPayload > maxPayload) {
    workingInput.value = maxPayload;
  }
}
    ...
    costMetrics.map((metric) => {
      ...
      const ratio =
        metric.direction === "lower"
          ? (reference - Math.max(value, 0)) / reference
          : (Math.max(value, 0) - reference) / reference;

      return clamp(ratio, 0, 1.5);
    }),
  );

  return Math.round(purchaseCost * pressure * 0.8);
}
...
function metricInputHint(metric) {
  if (metric.derived) return "자동 추산";
  if (!metric.devCost) return metricDirectionLabel(metric);
          ...
          return `
            <div class="${rowClass}">
              <label for="metric-${metric.id}">
                ${metric.label}
                <small>기준 ${baseline}${metric.unit} · ${metricInputHint(metric)}</small>
              </label>
              <input
                id="metric-${metric.id}"
                type="number"
...
function calculateEconomy() {
  ...
  return { score, payback, netBenefit, developmentCost, initialCost };
}
